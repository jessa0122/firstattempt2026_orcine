# Orcine

AI-assisted web application project using Vue.js

---

## Framework

Vue.js (Vite)

## Module

Donation Module

---

## Project Overview

This project is an **Alumni Donation Dashboard Web Application** designed for Ateneo de Davao University. It allows alumni and administrators to:

- Register and login
- Select user roles (Alumni / Administrator)
- View donation dashboard
- Create donation campaigns
- Track donations and progress
- View leaderboard rankings
- Receive notifications

The system is built using **Vue.js** and developed with the assistance of ChatGPT Plus.

---

## Installation Guide

Follow these steps to run the project on another computer:

git clone https://github.com/jessa0122/firstattempt2026_orcine.git  
cd firstattempt2026_orcine  
npm install  
npm run dev

Then open:  
http://localhost:5173

---

## AI Tools Used

- ChatGPT Plus

This tool is used for:

- generating UI and layout
- debugging Vue errors
- improving design and spacing
- implementing navigation flow
- refining and completing the project

---

## Prompt Used

**First Prompt:**  
Create a Vue.js alumni donation dashboard web application with login, role selection, and admin panel.

**Main Prompt:**  
Create a complete Vue.js web application that includes signup, forgot password, login, role selection, admin dashboard, donation campaigns, campaign creation, donation review, leaderboard, and notifications. Use a modern UI with blue and purple gradients and card layouts.

---

## Screenshots

### Signup Page

<img width="401" height="858" alt="01-signup png" src="https://github.com/user-attachments/assets/f8be0ee7-d34f-49a8-9a01-630763ac60aa" />

### Forgot Password

<img width="378" height="758" alt="02-forgot-password png" src="https://github.com/user-attachments/assets/faa126d1-f184-4212-bfee-7db0a44eee21" />

### Login Page

<img width="403" height="865" alt="03-login png" src="https://github.com/user-attachments/assets/d43b814c-cda7-43c0-9c31-d44dd36d18fc" />

### Role Selection

<img width="407" height="862" alt="04-role-selection png" src="https://github.com/user-attachments/assets/c2df1e5b-ed03-4589-9f69-9461f56bbe75" />

### Admin Dashboard

<img width="412" height="860" alt="05-admin-home png" src="https://github.com/user-attachments/assets/84d66fee-f96c-4c3f-b876-59a466603736" />

### Donation Campaigns

<img width="406" height="858" alt="06-donation-campaigns png" src="https://github.com/user-attachments/assets/64cec43d-2e7f-4e91-9030-2a9c239fd3d5" />

### Create Campaign

<img width="416" height="875" alt="07-create-campaign png" src="https://github.com/user-attachments/assets/01dde712-7170-4017-b7a6-0b06a7a94b4a" />

### Donation Review

<img width="411" height="863" alt="08-review png" src="https://github.com/user-attachments/assets/4bd70d0a-7111-4c87-bef1-2933eb243ded" />

### Leaderboard

<img width="396" height="865" alt="09-leaderboard png" src="https://github.com/user-attachments/assets/2107e199-9d4f-4ad8-9488-266ea9a3fa9e" />

### Notifications

<img width="412" height="866" alt="10-notifications png" src="https://github.com/user-attachments/assets/f39fc237-185b-4158-90c2-34e774706024" />

---

## Subjective Experience (AI Vibe)

Using AI tools made development faster and more efficient, and for me ChatGPT Plus is best for debugging, designing, and structured coding.

Overall, AI greatly improved productivity, but manual checking and adjustments were still necessary.

---

## Repository Link

https://github.com/jessa0122/firstattempt2026_orcine

---

## PWA Conversion (Activity #14)

This project was enhanced into a **Progressive Web Application (PWA)** following modern web application standards.

### Features Added

- Web App Manifest (installable application)
- Service Worker registration
- Offline caching using Workbox
- App icons (192x192 and 512x512)
- Installable desktop application support

---

### Why Service Worker is Needed

The service worker allows the application to cache important files such as HTML, CSS, JavaScript, and images. This enables the application to load even without an internet connection and improves performance by reducing network requests.

---

### Install Feature

The application can be installed directly from the browser and runs as a standalone desktop application, separate from the browser window.

---

## Master Prompt (AI Log)

I am using Vue.js with Vite. Help me convert my existing web application into a Progressive Web App (PWA). I need to generate a valid manifest.json, register a service worker, implement caching strategies so the app works offline, and configure app icons based on university branding.

---

## Hallucinations / Errors Fixed Manually

- The project README originally referenced Next JS, but the actual framework used is Vue.js (Vite).
- The index.html file was incomplete and missing the head section, which caused PWA warnings.
- Required icon files (icon-192.png and icon-512.png) were not initially present and had to be added manually in the public folder.
- Attempting offline testing using npm run dev resulted in ERR_INTERNET_DISCONNECTED errors because Vite development mode is not suitable for offline testing.
- The correct offline functionality was verified using npm run build and npm run preview.

---

## PWA Testing Summary

The application was successfully tested for:

- Installability (App installed via browser)
- Standalone execution (runs outside browser)
- Offline functionality (loads without internet connection)
- Service Worker caching (resources served from cache)
